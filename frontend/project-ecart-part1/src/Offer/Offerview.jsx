...
const Offerview = () => {
  ...
  function deleteOffer(offer_id) {
    fetch(`${config.URL}/offer/${offer_id}`, {
      method: "DELETE",
    }).then((res) => {
      if (res.status === 200) {
        // alert("userdeleted");
  ...
  function updateOffer() {
   ...
   let reqData = {
    ...
    fetch(`${config.URL}/offer/${offer_id}`, reqData)
    .then((result)=>{
    ...
    />
       <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          ...
          <select className="form-select" value={valid_in} onChange={(e) =>setValid_in(e.target.value)}>
            <option></option>
