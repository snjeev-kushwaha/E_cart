...
function submitData(Reg_no,shop_id,shop_name,address,country,state,city,pincode,contact,owner,type,email,url,gst,turnover,description,terms_condition,status,image){
  ...
  async function displayShop(){
    let response = await fetch(`${config.URL}/shop_registration`, {method: "GET"})
 ...
 // delete api
  ...
  function deleteShop(Reg_no){
      fetch(`${config.URL}/shop_registration/${Reg_no}`,{
        method:"DELETE"
      })
      .then((res)=>{
        if(res.status === 200){
  ...
  function updateShop() {
   ...
   let reqData = {
    ...
    fetch(`${config.URL}/shop_registration/${Reg_no}`, reqData)
    .then((result)=>{
  ...
  const columns = [
   ...
   {
    name: "Edit",
    cell: row=><Button variant="success" onClick={() => submitData(row.Reg_no,row.shop_id,row.shop_name,row.address,row.country,row.state,row.city,row.pincode,row.contact,row.owner,row.type,row.email,row.url,row.gst,row.turnover,row.description,row.terms_condition,row.status,row.image)}><i className="bi bi-pencil"></i></Button>
   },
   {
    name: "Delete",
    cell: row=>{ 
    return (<Button onClick={() => deleteShop(row.Reg_no)} variant="danger"><i className="bi bi-trash"></i></Button>)
   }
   }
  ]
  ...
  useEffect(() => {
    ...
    return (
      ...
      <Row>
        ...
        <Col xs={6} md={2}>
          <Button variant="success"><Link to="/shopadd" style={{color:"white",textDecoration:"none"}}><i className="bi bi-plus" style={{fontSize:"20px"}}></i>Add Shop</Link></Button>
        </Col>
      </Row>
        <DataTable
        columns={columns}
