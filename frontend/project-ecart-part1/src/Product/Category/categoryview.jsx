...
async function displayCategory(){
  let res = await fetch(`${config.URL}/product_category`);
...
function deleteOffer(category_id){
    fetch(`${config.URL}/product_category/${category_id}`,{
      method:"DELETE"
    })
    .then((res)=>{
      if(res.status === 200){
  ...
  function updateCategory() {
   ...
   let reqData = {
    ...
    fetch(`${config.URL}/product_category/${category_id}`, reqData)
    .then((result)=>{
  ...
  const columns =[
    ...
    {
      name: "Edit",
      cell: (row) =><Button variant="success" onClick={() => submitData(row.category_id,row.category_name)} style={{onmouseOver:"blue"}}><i className="bi bi-pencil"></i></Button>
    },
    {
      name:"Delete",
      cell:(row) =>{
        return (<Button onClick={() => deleteOffer(row.category_id)} variant="danger"><i className="bi bi-trash"></i></Button>)
      }
    }
  ]
...
setFilterCategories(result);
  ...
  return (
    ...
    <Row>
        ...
        <Col xs={6} md={3}>
        <Button variant="success"><Link to="/categoryadd" style={{color:"white",textDecoration:"none"}}><i className="bi bi-plus" style={{fontSize:"20px"}}></i>Addcategory</Link></Button>
        </Col>
      </Row>
      <DataTable
      columns= {columns}
